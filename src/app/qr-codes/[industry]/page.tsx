import { industryUseCases } from "@/lib/programmatic-seo";
...
export async function generateStaticParams() {
  return industryUseCases.map((c) => ({ industry: c.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const p = await params;
  const config = industryUseCases.find(c => c.slug === p.industry);
  if (!config) return {};
  return resolveMetadata(buildGeneratorMeta(config, `qr-codes/${config.slug}`));
}
